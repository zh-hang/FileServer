...
void Connection::sendMsg(int connfd, const std::string &msg) {
  fd_set fds;
  timeval timeout{6, 0};
  if (trySelect(fds, timeout) && send(connfd, msg.c_str(), BUFF_SIZE, 0))
    return;
...
std::string Connection::recvMsg(int connfd) {
  ...
  fd_set fds;
  timeval timeout{6, 0};
  if (selectDeal(fds, timeout) && recv(connfd, buf, BUFF_SIZE, 0))
  ...
  int Connection::trySelect(fd_set & readfds, fd_set & writefds,
  ...
  void UDPConnection::sendFileUDP(std::string filename, sockaddr * ra) {
  ...
  void UDPConnection::recvFileUDP(std::string filename) {
    char buf[BUFF_SIZE];
